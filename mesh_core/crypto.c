                        ...
                        uint32_t iv_index)
{
    ...
    err = mbedtls_chachapoly_auth_decrypt(&chachapoly, buf->len, nonce, ad, ad ? 16 : 0, tag, buf->data, out->data);
...
*/
void create_cha_cha_key(const uint8_t key[16], uint8_t *cha_key) {
